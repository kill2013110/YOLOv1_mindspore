...
def post_process(preds_topk, score_threshold, nms_iou_threshold, class_agnostic=False):
    '''
    cls_scores_topk [batch_size,max_num]
    cls_classes_topk [batch_size,max_num]
    boxes_topk [batch_size,max_num,4]
    '''
    NMS = ms.ops.NMSWithMask(nms_iou_threshold)

    _cls_scores_post = []
    _cls_classes_post = []
    _boxes_post = []
    cls_scores_topk, cls_classes_topk, boxes_topk = preds_topk
    cls_scores_topk = ms.numpy.squeeze(cls_scores_topk, axis=0)
    cls_classes_topk = ms.numpy.squeeze(cls_classes_topk, axis=0)
    boxes_topk = ms.numpy.squeeze(boxes_topk, axis=0)
    for batch in range(cls_classes_topk.shape[0]):
        ...
        if class_agnostic:
            boxes_for_nms = _boxes_b
        else:
            max_coordinate = _boxes_b.max()
            offsets = _cls_classes_b.astype(_boxes_b.dtype) * (max_coordinate + 1.).astype(_boxes_b.dtype)
            boxes_for_nms = _boxes_b + offsets[:, None]

        _, ind, keep_mask = NMS(ms.ops.cat((boxes_for_nms, _cls_scores_b.reshape(-1, 1)), 1))
        nms_ind = ind[keep_mask]
...
def box_nms(boxes, scores, thr):
    ...
    # order = torch.argsort(torch.Tensor(scores.asnumpy())).numpy()
    order = np.argsort(scores.asnumpy())
    ...
    while order.size > 0:
        ...
        order = order[inds + 1]
    return Tensor(keep, ms.int32)

def ClipBoxes(batch_imgs, batch_boxes):
    batch_boxes = ops.clip_by_value(batch_boxes, Tensor(0, ms.float32), Tensor(9999999, ms.float32))
    h, w = batch_imgs.shape[2:]
    batch_boxes[..., [0, 2]] = ops.clip_by_value(batch_boxes[..., [0, 2]], Tensor(0, ms.float32), w - 1)
    batch_boxes[..., [1, 3]] = ops.clip_by_value(batch_boxes[..., [1, 3]], Tensor(0, ms.float32), h - 1)
    return batch_boxes
