...
from .assigner_utils import unmap, anchor_inside_flags, multi_apply

def coords_fmap2orig(feature, stride):
    """
    与tood head 中的 anchor_center函数功能同
    ...
    coords = mnp.stack((shift_x, shift_y), -1)
...
class GenTargets(nn.Cell):
    ...
    def getTargets(self, inputs):
        ...
        cls_scores, reg_preds, anchors = inputs[0]
        gt_boxes = inputs[1]
        classes = inputs[2]
        ...
        for level in range(len(cls_scores)):
            level_out = (cls_scores[level], reg_preds[level], anchors[level])
            ...
            reg_targets_all_level = reg_targets_all_level + (level_targets[1],)
    ...
    def _gen_level_targets(self, out, gt_boxes, classes, stride, limit_range, sample_radiu_ratio=1.5):
        ...
        '''
        cls_scores, cnt_logits, reg_preds = out
        batch_size = cls_scores.shape[0]
        class_num = cls_scores.shape[1]
        m = gt_boxes.shape[1]
        transpose = ops.Transpose()
        reshape = ops.Reshape()
        cls_scores = transpose(cls_scores, (0, 2, 3, 1))  # [batch_size,h,w,class_num]
        coords = coords_fmap2orig(cls_scores, stride[0])  # [h*w,2]
        cls_scores = reshape(cls_scores, (batch_size, -1, class_num))  # [batch_size,h*w,class_num]
        ...
        tempmin = ops.Cast()(tempmin, ms.int32)
        tempmax = ops.Cast()(tempmax, ms.int32)
        tempMask_in_level = ops.Mul()(tempmin, tempmax)
        mask_in_level = ops.Cast()(tempMask_in_level, ms.bool_)

        radiu = stride[0] * sample_radiu_ratio
        ...
        tempingtboxes = ops.Cast()(mask_in_gtboxes, ms.int32)
        tempmaskinlevel = ops.Cast()(mask_in_level, ms.int32)
        tempmaskcenter = ops.Cast()(mask_center, ms.int32)
        ...
        x = mnp.arange(0, areas_min_ind[0].shape[0]).astype(ms.int32)
...
def compute_cls_loss(preds, targets, mask, MIN, MAX):
    ...
    ones = ops.Ones()
    candidate = ones(num_pos.shape, ms.float32)
...
def compute_cnt_loss(preds, targets, mask, MIN, MAX):
    ...
    for batch_index in range(batch_size):
        ...
        if pred_pos.shape[0] != 0:
            ...
            bce_result = mnp.zeros((1,), ms.float32)
...
def compute_reg_loss(preds, targets, mask, MIN, MAX, ZERO, ZB, mode='giou'):
    ...
    for batch_index in range(batch_size):
        ...
        if pred_pos.shape[0] != 0:
            ...
            loss_result = mnp.zeros((1,), ms.float32)
...
def giou_loss(preds, targets, mask_index, ZERO, ZB, MAX):
    ...
    zeros = ops.Zeros()
    candidate = zeros(wh_min.shape, ms.float32)
    ...
    zeros = ops.Zeros()
    candidate = zeros(wh_max.shape, ms.float32)
    wh_max = mnp.where(wh_max < 0, candidate, wh_max)
    G_area = wh_max[:, 0] * wh_max[:, 1]  # [n]
    zeros = ops.Zeros()
    candidate = zeros(G_area.shape, ms.float32) + 1e-10
...
class WithLossCell(nn.Cell):
    ...
    def construct(self, input_imgs, gt_bboxes, gt_classes):
        #preds
        out = self._model(input_imgs)
        import torch
        torch_out = torch.load('/mnt/f/liwenlong/mask_mmdet/cls_bbox_outs.npy', map_location=torch.device('cpu') )
        # stop gradients
        cls_scores = ()
        reg_preds = ()
        temp_cls_scores, temp_reg_preds, anchors = out
        for i in temp_cls_scores:
            cls_scores = cls_scores + (ops.Zeros()(i.shape, i.dtype),)
        for i in temp_reg_preds:
            reg_preds = reg_preds + (ops.Zeros()(i.shape, i.dtype),)
        stop_out = (cls_scores, reg_preds, anchors)
        targets = self._targets_fn.getTargets((stop_out, gt_bboxes, gt_classes))
