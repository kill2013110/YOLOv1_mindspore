...
def _conv2d_zeros_pad(in_channel, out_channel, kernel_size=1, stride=1,
                      padding=0, pad_mode='pad', weight_init=None):
    '''对齐pytorch的zeros pad
    resnet50 的所有卷积层均不含bias
    '''

    if padding == 0:
        return nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, stride=stride,
                      padding=0, pad_mode='valid', weight_init=weight_init)
    if padding > 0:
...
def _conv2d_pad(in_channel, out_channel, kernel_size=1, stride=1,
                      padding=0, pad_mode='pad', weight_init=None):
    return nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, stride=stride,
...
def _conv3x3(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    ...
    if res_base:
        return _conv2d(in_channel, out_channel, kernel_size=3, stride=stride,
...
def _conv1x1(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    if use_se:
        ...
        weight = Tensor(kaiming_normal(weight_shape, mode="fan_out", nonlinearity='relu'))
    if res_base:
        return _conv2d(in_channel, out_channel, kernel_size=1, stride=stride,
...
def _conv7x7(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    if use_se:
        ...
        weight = Tensor(kaiming_normal(weight_shape, mode="fan_out", nonlinearity='relu'))
    if res_base:
        return _conv2d(in_channel, out_channel,
                         kernel_size=7, stride=stride, padding=3, pad_mode='pad', weight_init=weight)
    return _conv2d(in_channel, out_channel,
                     kernel_size=7, stride=stride, padding=0, pad_mode='pad', weight_init=weight)
...
class ResNet(nn.Cell):
    ...
    def __init__(self,
        ...
        self.bn1 = nn.BatchNorm2d(64, momentum=0.9, use_batch_statistics=FROZEN, ).set_train(FROZEN)
        self.relu = P.ReLU()
    ...
    def construct(self, x):
        # print(self.conv1)
        # a = ms.Tensor(np.load('/mnt/f/liwenlong/mask_mmdet/x.npy'))
        # x = a.copy()
...
def resnet18(pretrained=False, preckpt_path=None, class_num=1000):
    """
    ...
    Examples:
        >>> net = resnet18(10)
    """
    model = ResNet(ResidualBlockBase,
                  [2, 2, 2, 2],
                  [64, 64, 128, 256],
                  [64, 128, 256, 512],
                  [1, 2, 2, 2],
    ...
    if pretrained:
        if preckpt_path is not None:
            ...
            info = load_param_into_net(model, param_dict, strict_load=True)
            print(f'------------ resnet18 param_not_load: {info[0]}-----------------')
            print(f'------------ resnet18 ckpt_not_load: {info[1]}-----------------')
        else:
            print('------------ Not load ResNet50 pretrain ckpt !-----------------')
    return model

def resnet34(pretrained=False, preckpt_path=None, class_num=1000):
    """
    ...
    if pretrained:
        if preckpt_path is not None:
            param_dict = load_checkpoint(preckpt_path)
            info = load_param_into_net(model, param_dict, strict_load=True)
            print(f'------------ resnet34param_not_load: {info[0]}-----------------')
            print(f'------------ resnet34 ckpt_not_load: {info[1]}-----------------')
        else:
            print('------------ Not load ResNet34 pretrain ckpt !-----------------')
    return model

def resnet50(pretrained=False, preckpt_path=None, class_num=1000):
    model = ResNet(ResidualBlock, [3, 4, 6, 3], [64, 256, 512, 1024], [256, 512, 1024, 2048], \
    [1, 2, 2, 2], class_num,
                   res_base=True ##修改为True
                   )
    if pretrained:
        if preckpt_path is not None:
            param_dict = load_checkpoint(preckpt_path)
            info = load_param_into_net(model, param_dict, strict_load=True)
            print(f'------------ resnet50 param_not_load: {info[0]}-----------------')
            print(f'------------ resnet50 ckpt_not_load: {info[1]}-----------------')
        else:
            print('------------ Not load ResNet50 pretrain ckpt !-----------------')
    return model
